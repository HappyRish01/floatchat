  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      connections.forEach(connection => {
        ...
        // Ensure we're working with string color values
        const fromColor = String(from.isHub ? theme.palette.secondary.main : from.color);
        const toColor = String(to.isHub ? theme.palette.secondary.main : to.color);
        
        // Convert to rgba to properly handle transparency
        let fromColorWithAlpha, toColorWithAlpha;
        
        // Handle various color formats
        if (fromColor.startsWith('#')) {
          fromColorWithAlpha = hexToRgba(fromColor, 0.5);
        } else if (fromColor.startsWith('rgb(')) {
          fromColorWithAlpha = fromColor.replace('rgb(', 'rgba(').replace(')', ', 0.5)');
        } else if (fromColor.startsWith('rgba(')) {
          // Already has alpha, just use it
          fromColorWithAlpha = fromColor;
        } else {
          // Default fallback
          fromColorWithAlpha = 'rgba(0, 184, 212, 0.5)';
        }
        
        if (toColor.startsWith('#')) {
          toColorWithAlpha = hexToRgba(toColor, 0.5);
        } else if (toColor.startsWith('rgb(')) {
          toColorWithAlpha = toColor.replace('rgb(', 'rgba(').replace(')', ', 0.5)');
        } else if (toColor.startsWith('rgba(')) {
          // Already has alpha, just use it
          toColorWithAlpha = toColor;
        } else {
          // Default fallback
          toColorWithAlpha = 'rgba(30, 136, 229, 0.5)';
        }
          
        gradient.addColorStop(0, fromColorWithAlpha);
        gradient.addColorStop(1, toColorWithAlpha);
